...
if (!empty($_POST)) {
    ...
    if ($validacao) {
        ...
        $sql = "UPDATE servico  set nome = ?, descricao = ?, disponibilidade = ?, Unidade_idunidade= ? WHERE idservico = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($nome, $descricao, $disponivel, $unidade, $id));
        Banco::desconectar();
    ...
    $pdo = Banco::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "Select * from servico inner JOIN unidade on servico.Unidade_idunidade= unidade.idunidade where idservico = ? ";
